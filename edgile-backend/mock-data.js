...
const { ClassroomAssignment } = require('./src/models/ClassroomAssignment');
...
async function createMockData() {
  try {
    ...
    // Delete any old classroom assignments to ensure our classrooms are "available"
    const currentYear = new Date().getFullYear();
    const academicYear = `${currentYear}-${currentYear + 1}`;
    
    const deleted = await ClassroomAssignment.deleteMany({ 
      university: universityId,
      academicYear
    });
    
    console.log(`Deleted ${deleted.deletedCount} old classroom assignments`);
